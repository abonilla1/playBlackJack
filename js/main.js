...
standBtn.addEventListener("click", () => {
  ...
  dealersTurn(dealerHand, gameDeck);
  checkForDealerWin(dealerPoints, playerPoints);
});
...
function shuffle(deck) {
  //will shuffle using Fisher-Yates method
  let m = deck.length,
    temp,
    idx;
  while (m) {
...
function renderEndRound() {
  if (player.blackjack && playerHand.length == 2) {
    ...
    messageOutput.innerHTML =
      "!!!5 Card Charlie!!! Click Next Hand to Continue";
    dealBtn.disabled = true;
    hitBtn.disabled = true;
    confetti.start(5000);
    animateWin();
    ...
    messageOutput.innerHTML =
      "BUST!!! Click Next Hand to Continue, Quit to Quit";
    calcPayout();
    dealBtn.disabled = true;
    hitBtn.disabled = true;
...
function animateWin() {
    document.querySelector(".ml12").classList.remove("special")
    anime
      .timeline({ loop: true })
      .add({
        targets: ".ml12 .letter",
